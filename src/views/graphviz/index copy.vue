<template>
  <div id="physicalGraph" class="physical-graph" />
</template>
...
export default {
  ...
  methods: {
    initEvent() {
      const graph = d3.select('#physicalGraph')
      graph
    ...
    renderGraph(idcData) {
      ...
      const svg = d3.select('#physicalGraph').select('svg')
      svg
        .attr('width', divWidth)
    ...
    genChildren(idcData) {
      ...
      if (layers.size) {
        layers.forEach(layer => {
          ...
          const n = layers.size
          const lg = (height - 3) / n
          const ll = (width - 0.5 * layer.length) / layer.length
          layer.forEach(zone => {
            let label
    ...
    setChildren(node, p1, pw, ph, tfsize, level) {
      const graph = d3.select('#physicalGraph').select(`#g_${node.guid}`)
