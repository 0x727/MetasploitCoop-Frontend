...
<script>
import { delRcScripts, fetchRcScripts } from '@/api/scripts'
...
export default {
  ...
  data() {
    return {
      ...
      defaultColumn: [
        ...
        { label: 'filename', width: 0, name: '文件名' }
      ],
      cmList: []
    }
  },
  ...
  methods: {
    ...
    async deleteRow(row) {
      await delRcScripts(row.id).then(response => {
        if (response) {
          this.$notify({
            title: '删除关键词成功',
    ...
    async fetchData(page = 1, size = 15) {
      ...
      await fetchRcScripts({ size: size, page: page }).then(response => {
